...
## 15.2 설정 모달 역할
설정 모달은 Home 과 Workout 에서 공통 사용한다.
...
## 15.3 프리뷰 직접 편집
Home 프리뷰 표에서는 다음 숫자 영역을 직접 수정할 수 있다.

- kg
- set
- rep

수정값은 `previewOverrides` 상태에 저장되고, 운동 시작 시 실제 루틴 생성에 반영된다.

## 15.4 하단 CTA 규칙
Home 화면의 `내 루틴 시작하기` 버튼은 스크롤과 무관하게 항상 화면 하단에 고정한다.

## 15.5 운동 중 설정 재적용
Workout 상태에서 설정 모달을 열어 다음 항목을 수정할 수 있다.
...
수정 후 현재 운동을 제외한 이후 루틴에 재적용할 수 있어야 한다.
